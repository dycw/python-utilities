...
class _CopyOrMoveDestinationExistsError(CopyOrMoveError):
    ...
    @override
    def __str__(self) -> str:
        return f"Cannot {self.mode} source {repr_str(self.src)} since destination {str(self.dest)!r} already exists"
...
@contextmanager
def yield_temp_path(path: PathLike, /, *, overwrite: bool = False) -> Iterator[Path]:
