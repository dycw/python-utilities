...
def plot_curve(
    ...
    *,
    ...
    try:
        ...
        msg = f"{array.name=}"
        raise PlotCurveNameNotStringError(msg) from None
    if len(vdim) == 0:
        msg = f"{array.name=}"
        raise PlotCurveNameEmptyError(msg) from None
    if label is None:
        label = vdim
...
class PlotCurveNameNotStringError(Exception):
    ...


class PlotCurveNameEmptyError(Exception):
    ...


__all__ = [
    "plot_curve",
    "PlotCurveNameEmptyError",
    "PlotCurveNameNotStringError",
]
