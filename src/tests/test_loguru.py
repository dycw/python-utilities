...
class TestAugmentLevels:
    def test_none(self: Self) -> None:
        result = _augment_levels()
        assert result == {}

    @given(levels=dictionaries(text_ascii(min_size=1), sampled_from(LogLevel)))
    def test_main(self: Self, *, levels: Mapping[str, LogLevel] | None) -> None:
...
class TestGetFilesPath:
    ...
    def test_both(
        self: Self, *, files: str, env_var_key: str, env_var_value: str
    ) -> None:
