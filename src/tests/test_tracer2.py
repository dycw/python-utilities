...
class TestTracer:
    ...
    async def test_async(self) -> None:
        ...
        tree = one(trees)
        root: Node = tree[tree.root]
        self._check_node(root, "tests.test_tracer2", "async_outer1", 0.04, "success")
        mid1, mid2 = cast(list[Node], tree.children(root.identifier))
        self._check_node(mid1, "tests.test_tracer2", "async_mid1", 0.01, "success")
        self._check_node(mid2, "tests.test_tracer2", "async_mid2", 0.02, "success")
        assert len(tree.children(mid1.identifier)) == 0
        (inner,) = cast(list[Node], tree.children(mid2.identifier))
        self._check_node(inner, "tests.test_tracer2", "async_inner", 0.01, "success")
