...
class TestTracer:
    def test_main(self) -> None:
        ...
        self._check_node(root, "tests.test_tracer", "outer1", 0.04, 0.3, "success")
        mid1, mid2 = cast(list[Node], tree.children(root.identifier))
        self._check_node(mid1, "tests.test_tracer", "mid1", 0.01, 0.3, "success")
        self._check_node(mid2, "tests.test_tracer", "mid2", 0.02, 0.3, "success")
        assert len(tree.children(mid1.identifier)) == 0
        (inner,) = cast(list[Node], tree.children(mid2.identifier))
        self._check_node(inner, "tests.test_tracer", "inner", 0.01, 0.3, "success")
    ...
    def test_multiple_calls(self) -> None:
        ...
        for tree in trees:
            root: Node = tree[tree.root]
            self._check_node(root, "tests.test_tracer", "inner", 0.01, 0.3, "success")
    ...
    def test_error(self) -> None:
        ...
        tree = one(trees)
        root: Node = tree[tree.root]
        self._check_node(root, "tests.test_tracer", "fails", 0.01, 0.3, "failure")
